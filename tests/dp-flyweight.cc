...
namespace hicc::dp::flyweight::wordprocessor {
    ...
    struct attr_holder : hicc::util::singleton<attr_holder> {
        attr_holder(hicc::util::singleton<attr_holder>::token) {}
        std::vector<std::shared_ptr<attr>> attrs;
    };
...
void test_flyweight_wordprocessor() {
    using namespace hicc::dp::flyweight::wordprocessor;
}

int main() {

    HICC_TEST_FOR(test_flyweight_basic);
    HICC_TEST_FOR(test_flyweight_meta);
    HICC_TEST_FOR(test_flyweight_wordprocessor);
