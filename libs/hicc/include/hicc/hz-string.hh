...
#ifndef HICC_CXX_HZ_STRING_HH
#define HICC_CXX_HZ_STRING_HH
...
namespace hicc::text {
  ...
  public:
    explicit jaro_winkler_distance(distance threshold = 0.7, bool case_sensitive = true)
        : _threshold(threshold), _case_sensitive(case_sensitive) {}
...
namespace hicc {
  namespace traits {
    ...
    template<typename T>
    struct has_write<T, decltype(void(std::declval<T &>().write(std::declval<std::ostream &>())))> : std::true_type {};
  ...
  inline std::string to_string(T const &t) {
    std::stringstream ss;
    // if constexpr (traits::has_write<T>::value)
    //   t.write(ss);
    // else
    ss << t;
    return ss.str();
